...
import sys
import SoGMF_BK, SoMLP_BK
import argparse


#################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="Run BXNeuMF1_3_1.")
    ...
    parser.add_argument('--mlp_pretrain', nargs='?', default='Pretrain/ml-1m_BXMLP1_3.h5',
...
def get_model(num_users, num_items, mf_dim=10, layers=[10], reg_layers=[0], reg_mf=0):
    ...
    #                                       name="mlp_embedding_user_social",
    #                                       init=init_normal, W_regularizer=l2(reg_layers[0]), input_length=1)
...
if __name__ == '__main__':
    ...
    print("BXNeuMF1_3_1 arguments: %s " % (args))
    ...
    if learner.lower() == "adagrad":
        model.compile(optimizer=Adagrad(lr=learning_rate), loss='binary_crossentropy', loss_weights=[1, 0.2])
    elif learner.lower() == "rmsprop":
        model.compile(optimizer=RMSprop(lr=learning_rate), loss='binary_crossentropy', loss_weights=[1, 0.2])
    elif learner.lower() == "adam":
        model.compile(optimizer=Adam(lr=learning_rate), loss='binary_crossentropy', loss_weights=[1, 0.2])
    else:
        model.compile(optimizer=SGD(lr=learning_rate), loss='binary_crossentropy', loss_weights=[1, 0.2])

    # Load pretrain model
    if mf_pretrain != '' and mlp_pretrain != '':
        gmf_model = SoGMF_BK.get_model(num_users, num_items, mf_dim)
        gmf_model.load_weights(mf_pretrain)
        mlp_model = SoMLP_BK.get_model(num_users, num_items, layers, reg_layers)
        mlp_model.load_weights(mlp_pretrain)
        model = load_pretrain_model(model, gmf_model, mlp_model, len(layers))
        print("Load pretrained BXGMF1_3 (%s) and BXMLP1_3 (%s) models done. " % (mf_pretrain, mlp_pretrain))
        # print("Load pretrained BXGMF1_3 (%s) and BXMLP1_3 (%s) models done. " % (mf_pretrain, mlp_pretrain), file=f)
    # Init performance
    ...
    if args.out > 0:
        print("The best XNeuMF model is saved to %s" % (model_out_file))
